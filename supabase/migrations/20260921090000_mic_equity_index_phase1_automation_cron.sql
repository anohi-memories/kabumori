-- Market Intelligence Core (MIC) Equity Index Phase 1: State evaluator Cron
-- automation.
--
-- Same safety pattern as the existing MIC automation migrations
-- (20260916100000_mic_phase1b_automation_cron.sql,
-- 20260919090000_mic_fx_phase1_automation_cron.sql) -- see those files for
-- the full rationale. Summary:
--
-- 1. No new ingest Cron job is added here. Equity Index metrics
--    (NIKKEI225/SP500/NASDAQCOMPOSITE/NASDAQ100/VIX) are all fetched by the
--    existing FRED adapter, so the existing FRED ingest jobs
--    (mic-ingest-fred-2100, mic-ingest-fred-0100) are reused as-is and are
...
--    only -- the same secret the rates/commodities/fx evaluator jobs
--    already use. That secret's value is not touched by this migration.
...
--    ever made.
-- 4. Each job's body names exactly one domain (`{"domains":["equity_index"]}`),
--    never all-domain.
...
--    FRED run correctly blocks the equity_index evaluator even if an older
--    completed run is still inside the 60-minute window. This is the exact
--    hardened gate already used by mic-evaluator-rates-after-fred-* (which
--    also gates on source_key='fred'), reused unchanged here.
-- 6. `cron.schedule(name, ...)` upserts by job name: re-running this
--    migration is safe, and it only ever references the two new
--    `mic-evaluator-equity-index-after-fred-*` job names below, so no
--    existing MIC or non-MIC Cron job (including the FRED ingest jobs and
--    the existing rates evaluator jobs that also fire off the same FRED
--    ingest) is touched.
...
--    MIC domain.
--
-- Schedule rationale: mirrors mic-evaluator-rates-after-fred-* exactly,
-- since both rates and equity_index are populated by the same FRED ingest
-- jobs -- 15 minutes after each FRED ingest slot (21:00 Mon-Fri, 01:00
-- Tue-Sat UTC).

select cron.schedule(
  'mic-evaluator-equity-index-after-fred-2100',
  '15 21 * * 1-5',
  $$
  with secret as (
  ...
  select net.http_post(
    ...
    body := '{"domains":["equity_index"]}'::jsonb,
...
select cron.schedule(
  'mic-evaluator-equity-index-after-fred-0100',
  '15 1 * * 2-6',
  $$
  with secret as (
    select decrypted_secret
