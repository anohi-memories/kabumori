create table if not exists public.close_report_settings (
  id boolean primary key default true check (id),
  is_active boolean not null default false,
  window_start time not null default '15:58',
  center_time time not null default '16:00',
  window_end time not null default '16:02',
  futures_target_time time not null default '15:45',
...
create table if not exists public.close_report_runs (
  ...
  nikkei_data jsonb not null default '{}'::jsonb,
  topix_data jsonb not null default '{}'::jsonb,
  nikkei_futures_1545_data jsonb,
...
create index if not exists close_report_runs_scheduled_at_idx
  on public.close_report_runs (scheduled_at desc);

alter table public.close_report_settings enable row level security;
alter table public.close_report_runs enable row level security;
grant select on public.close_report_settings to service_role;
grant select, insert, update on public.close_report_runs to service_role;

insert into public.close_report_settings (id, is_active)
values (true, false)
on conflict (id) do update set
  window_start = '15:58', center_time = '16:00', window_end = '16:02',
  futures_target_time = '15:45', timezone = 'Asia/Tokyo',
  holiday_edition_enabled = false, is_active = false, updated_at = now();

insert into public.posting_windows
  (post_type, slot_no, start_time, end_time, timezone, daily_probability, is_active)
values ('close_report', 1, '15:58', '16:02', 'Asia/Tokyo', 1, false)
...
create or replace function public.plan_close_report(
...
as $$
declare
  settings public.close_report_settings%rowtype;
  candidate timestamptz;
begin
  select * into settings from public.close_report_settings where id = true;
  ...
  values (p_date, 'close_report', 1, candidate)
  on conflict (schedule_date, post_type, slot_no) do nothing;

  return query
  select * from public.scheduled_posts
  where schedule_date = p_date and post_type = 'close_report'
...
begin
  ...
  if selected_run.id is null then raise exception 'CLOSE_REPORT_RUN_NOT_FOUND'; end if;
...
revoke all on function public.plan_close_report(date) from public;
revoke all on function public.complete_close_report_post(uuid, uuid, text) from public;
revoke all on function public.claim_due_post() from public;
grant execute on function public.plan_close_report(date) to service_role;
grant execute on function public.complete_close_report_post(uuid, uuid, text) to service_role;
grant execute on function public.claim_due_post() to service_role;

-- Installed for verification only. Publication remains disabled until explicit approval.
update public.close_report_settings set is_active = false, updated_at = now() where id = true;
update public.posting_windows set is_active = false where post_type = 'close_report';
