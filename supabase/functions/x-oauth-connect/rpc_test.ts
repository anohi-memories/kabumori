  ...
  await assert.rejects(
    () =>
      rpc(
        ...
        "failed_rpc",
        {},
        async () => new Response("private error details", { status: 500 }),
      ),
    (error: unknown) =>
      error instanceof BrandContextError &&
      error.message === "OAUTH_CONNECTION_DB_WRITE_FAILED",
  );
});
...
test("OAuth start continues after a void RPC and returns the read-only authorization response", async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    assert.equal(
      String(input),
      `${SUPABASE_URL}/rest/v1/rpc/begin_ai_salaryman_lab_oauth_connection`,
    );
...
test("Kabumori OAuth start omits handle from its account-specific RPC and returns write scopes", async () => {
  const fetchImpl: typeof fetch = async (input, init) => {
    assert.equal(
      String(input),
      `${SUPABASE_URL}/rest/v1/rpc/begin_kabumori_oauth_recovery`,
    );
    const body = JSON.parse(String(init?.body)) as Record<string, unknown>;
