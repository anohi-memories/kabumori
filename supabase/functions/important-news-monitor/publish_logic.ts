...
export function checkPublishCandidate(
  ...
  const checks = {
    ...
    // Both safe importance tiers may auto-publish. Queue selection still prioritizes
    // most_important before important, while the rate-control gate limits publication frequency.
...
export async function publishImportantNewsCandidate(
  ...
  if (!candidate || candidate.status !== "ready_for_publish" ||
    !["important", "most_important"].includes(candidate.importance)) {
