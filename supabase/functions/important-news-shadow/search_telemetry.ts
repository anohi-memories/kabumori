...
export type TargetedSearchUsage = {
  inputTokens: number;
  outputTokens: number;
...
/** Sum one successful response into the run aggregate using the unchanged per-response estimate. */
export function aggregateTargetedSearchUsage(
  total: TargetedSearchUsage,
  next: TargetedSearchUsage,
): TargetedSearchUsage {
  return {
    inputTokens: total.inputTokens + next.inputTokens,
    outputTokens: total.outputTokens + next.outputTokens,
    webSearchCalls: total.webSearchCalls + next.webSearchCalls,
