...
test("startConnection: rejects a malformed state_hash before ever calling the RPC", async () => {
  let rpcCalls = 0;
  await assert.rejects(
    () => startConnection({
      request: { stateHash: "not-a-hash", codeChallenge: "c", redirectUri: "app://cb" },
...
test("startConnection: forwards the user's own JWT to the RPC (never service_role/anon-only), and builds the authorize URL from the RPC's returned brand/account", async () => {
  const stateHash = await hashState("client-generated-raw-state");
  let capturedAuth = "";
  let capturedBody: Record<string, unknown> = {};
  const result = await startConnection({
    request: { stateHash, codeChallenge: "challenge-value", redirectUri: "kabumori-social-mobile://oauth-callback" },
  ...
  assert.equal(capturedBody.p_state_hash, stateHash);
  ...
  assert.equal(url.searchParams.get("state"), stateHash);
...
test("startConnection: a cross-user or no-membership rejection from the RPC (e.g. ownership DB error) propagates as a real failure, not a fabricated success", async () => {
  const stateHash = await hashState("s");
  await assert.rejects(
    () => startConnection({
      request: { stateHash, codeChallenge: "c", redirectUri: "app://cb" },
      userAccessToken: "user-token", supabaseUrl: SUPABASE_URL, anonKey: ANON_KEY, clientId: CLIENT_ID,
    ...
    () => completeConnection({
      ...
      fetchImpl: async (input) => {
        ...
        if (url.includes("consume_social_mobile_x_oauth_state")) {
          return Response.json([{ redirect_uri: "app://cb-original", brand_id: "u_x", social_account_id: "sa_x" }]);
        }
...
test("completeConnection: an unconsumable state (replay, wrong user, expired, or unknown) is rejected before any token exchange", async () => {
  let tokenExchangeCalls = 0;
  await assert.rejects(
    () => completeConnection({
...
test("completeConnection: happy path exchanges the code, reads identity, completes via the RPC forwarding the user JWT, and never returns a token in its result", async () => {
  const calls: string[] = [];
  const result = await completeConnection({
    ...
    fetchImpl: async (input, init) => {
      ...
      if (url.includes("consume_social_mobile_x_oauth_state")) {
        ...
        return Response.json([{ redirect_uri: "kabumori-social-mobile://oauth-callback", brand_id: "u_abcdef", social_account_id: "sa_abcdef" }]);
      }
      ...
      if (url.includes("complete_social_mobile_x_oauth_connection")) {
        ...
        assert.equal(body.p_social_account_id, "sa_abcdef");
  ...
  await assert.rejects(
    () => completeConnection({
      ...
      fetchImpl: async (input) => {
        const url = String(input);
        if (url.includes("consume_social_mobile_x_oauth_state")) {
          return Response.json([{ redirect_uri: "app://cb", brand_id: "u_x", social_account_id: "sa_x" }]);
        }
