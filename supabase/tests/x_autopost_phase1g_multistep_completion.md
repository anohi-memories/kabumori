...
- The scheduled greeting date must equal the execution day's JST date at claim acquisition and again before each provider step. An overdue prior-day row fails closed (`GREETING_SCHEDULE_DATE_STALE`) rather than posting stale greeting content today under a separate prior-day claim. A future dispatcher must settle/flag that row without sending X media/post requests.
