...
## Latest H1 result — important-news-monitor caller-auth source candidate (2026-09-23)

- task_id: `kabumori-important-news-monitor-caller-auth-remediation-candidate-20260923`
- result: `review_required` — source/config candidate is ready for C1; production migration, Vault write, Function secret/config change, and deployment require separate approval.
- authorization_scope: the user explicitly approved designing and implementing the coordinated source candidate for caller authentication on exactly the four `important-news-monitor` pg_cron jobs, safe secret storage references, and Function-side validation. The user explicitly prohibited changing post content, Cron frequency, `auto_publish` conditions, and news judgement logic.
- production_callers: read-only inspection found four monitor jobs: fetch (20-minute slots with existing settings/date predicates), judgement (20-minute slots), generation (20-minute slots), and publish-ready (every 5 minutes with the existing `auto_publish` gate). Their existing bodies, URL and WHERE predicates were inspected with URL/header literals redacted. The separate `important-news-shadow` job was excluded. No app/admin direct caller was found in the inspected source.
- design: a dedicated random 32-byte base64url credential is sent in `x-important-news-cron-secret`; Function secret `IMPORTANT_NEWS_CRON_SECRET` is checked in constant-time before loading service-role credentials, parsing JSON, or dispatching any mode. Cron commands read the same value at execution time from Vault entry `important_news_monitor_cron_secret`. The migration stores no credential value and preflights the Vault entry and exact four job set. Keep `verify_jwt=false`; the dedicated handler check authenticates pg_net and all manual/dry-run modes.
- preserved: the migration patches only each existing command's JSONB headers expression and calls `cron.alter_job` with only `command`. It does not rebuild schedules, bodies, URLs, active state, settings/date predicates, or publication conditions. No news logic, post copy, X OAuth, Push, or other Function is touched.
- changed_files: `supabase/functions/important-news-monitor/index.ts`; new `caller_auth.ts`, `caller_auth_test.ts`, `caller_auth_wiring_test.ts`, `caller_auth_migration_test.ts`; new `supabase/migrations/20260923110440_important_news_monitor_caller_auth.sql`; new `docs/runbooks/important-news-monitor-caller-auth.md`.
- tests: targeted Node tests passed 7/7 using a temporary copy of the exact candidate files; `deno check caller_auth.ts` passed. Vercel preview status completed successfully. Full Important News regression suite and execution of the migration against a disposable PostgreSQL database were not run.
- compatibility_risks: the caller secret must be the same 43-character high-entropy value in the Function secret store and Vault. Any mismatch or missing value rejects requests. The migration intentionally fails if the four jobs or header expression differ from the inspected production shape. Rotate both secret stores in a separately approved coordinated operation.
- production_mutation: 0. No production SQL, Vault, Function settings, deployment, invocation, X post, or Push was performed.
- commit_hash: `7f089ca57fa47b57f2621c8ce7e0db01796f288e`
- push: source branch pushed; draft PR [#12](https://github.com/anohi-memories/kabumori/pull/12), head `7f089ca57fa47b57f2621c8ce7e0db01796f288e`.
- deploy: none.
