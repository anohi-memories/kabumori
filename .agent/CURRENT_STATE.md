...
- multibrand_work:
  ...
  - production AI Lab scopeは `tweet.read users.read offline.access` のread-onlyを維持。候補`a469dcc7`はAI Labだけを`tweet.read users.read tweet.write offline.access`へ変更し、`media.write`は追加しない。Kabumori scopeは変更なし
...
- parallel_work:
  - Codex H1 Phase 3KはPhase Aでsource変更が必要と判明したため、候補`a469dcc7`をpushしC1待ちで停止。Phase B/Cには未着手。H2/G2は対象を明示的に分離しており競合なし。
