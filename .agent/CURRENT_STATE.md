...
- active_workstream:
  - Codex slot 1: `review_required` — `x-multibrand-phase3k-ai-lab-first-live-test-20260916`。`x-oauth-connect` v18を承認candidateと12/12 byte一致でdeployし、AI Labを`tweet.read users.read tweet.write offline.access`で再認可。`kaishain_ai_lab`本人確認後、自然Cronで制御投稿1件が1回成功（X id `2100162295930511602`、115文字、fingerprint保存）。成功後のみ既存10枠を有効化し、19:14 JSTの自然plannerは期限切れslot 1–7をbackfillせず、slot 8–10だけを将来時刻へpending生成。C1確認待ち。
...
- parallel_work:
  - Codex H1 Phase 3KはPhase A/B/C完了、`review_required`。H2/G2は対象を明示的に分離しており競合なし。
