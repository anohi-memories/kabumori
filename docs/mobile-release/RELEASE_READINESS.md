...
| **Official app icon integrated** (2026-09-26, superseding the 2026-09-25 approved icon with a newer final design) | `assets/images/icon.png`, `app.json` → `expo.icon`/`ios.icon`; new master preserved at `assets/branding/kabumori-icon-master-2026-09-26.png` (the 2026-09-25 master is left in place, superseded, no longer pinned by tests). Verified with a real `expo prebuild`. **Not yet seen on an actual iPhone home screen** — that still needs a separately authorized real-device build. |
| **Kabumori-branded launch screen** (native splash + `AnimatedSplashOverlay`), now showing the official icon | `app.json` → `expo-splash-screen` plugin (`image: ./assets/images/icon.png`, `backgroundColor: #eef3ed`, `imageWidth: 200`); `src/components/animated-icon.tsx`. Same icon, same background, same size in both phases, so the handoff between them is seamless. Gentle fade + slight scale-down exit (600ms); skipped entirely when the OS Reduce Motion setting is on. Verified with a real `expo prebuild`. **Not yet seen on an actual iPhone** — real-device visual acceptance still pending. |
| **First-run onboarding (v1)**: 3 approved full-screen pages, shown once before the normal auth flow | `assets/onboarding/{01-brand,02-ai-analysis,03-report}.png`, `src/components/onboarding-screens.tsx`, `src/hooks/use-onboarding.ts` (`kabumori:onboarding:v1` in AsyncStorage). Native page dots; an image-relative `Pressable` hit target over the approved page-3 CTA artwork. Page-2's animated progress-bar overlay (task section D) was **not** implemented — see the note below the table. Verified with a real `expo prebuild`/web export. **Not yet seen on an actual iPhone** — real-device visual acceptance still pending. |
...
**Onboarding page 2's animated progress-bar overlay was deliberately not implemented** (`kabumori-onboarding-icon-integration-20260926`, task section D). The approved artwork's own static bar was kept exactly as designed. Reason: overlaying a native indeterminate bar in the exact position of the baked-in bar requires the overlay's screen-space rect to track the artwork precisely across every supported iPhone size, verified visually — something that cannot be responsibly confirmed from source alone, and a misaligned patch would visibly damage approved artwork, which is explicitly worse than not adding the animation. The task's own instructions name this exact situation as acceptable to defer. If it's wanted later, `src/lib/onboarding-layout.ts`'s `imageFractionToContainerRect()` (already used for the page-3 CTA hit target, and already unit-tested) is the mechanism to reuse; it only needs the bar's fractional region measured precisely and a real-device check.
