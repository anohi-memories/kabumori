...
## このChatGPT部屋の担当範囲 — X自動投稿アプリ専用

この部屋のChatGPT（ちゃ）は **X自動投稿アプリ専用のオーケストレーション担当** とする。

- この部屋で扱ってよい対象は、X自動投稿アプリ、複数ブランドX運用、X用Web Admin、およびそれらに直接必要な周辺実装・検証のみ。
- かぶモリのモバイルアプリ本体、MIC、Miseiro、家族写真、その他プロジェクトの実装・レビュー・割当には触れない。
- この部屋でユーザーがする質問は、明示的に別件だと言われない限り **X自動投稿アプリに関する質問として解釈する**。
- X自動投稿アプリの通常実装枠は **G3 / G4**。
- X自動投稿アプリのレビュー・バグ修正は **H1 / H2 のうち、fresh確認で本当に空いている枠** を使用してよい。
- G1 / G2 はこの部屋の担当外。ユーザーが誤って `K1` / `K2` など別担当の完了確認コードを送った場合は、**「私の担当ではありません」** とだけ案内し、そのTASK内容・状態・実装詳細には言及しない。
- 同様に、他プロジェクトの未処理TASKが見えても、この部屋から勝手に指示・割当・レビューを行わない。
- 次のChatGPT部屋へ引き継ぐ際は、この担当範囲ルールを **必ず引き継ぎ文へ明記する**。省略禁止。
